...
pub const OptimizedBuffer = struct {
    ...
    pub fn blendCells(self: *const OptimizedBuffer, overlayCell: Cell, destCell: Cell) Cell {
        ...
        if (hasBgAlpha or hasFgAlpha) {
            ...
            return Cell{
                ...
                .fg_tag = finalFgTag,
                .bg_tag = blendedTag(hasBgAlpha, overlayCell.bg_tag),
    ...
    fn drawTextBufferInternal(
        ...
        for (virtual_lines[firstVisibleLine..lastPossibleLine], 0..) |vline, slice_idx| {
            ...
            var lineFgTag: ColorTag = if (defaults.fg != null) defaults.fg_tag else ansi.COLOR_TAG_RGB;
            var lineBgTag: ColorTag = if (defaults.bg != null) defaults.bg_tag else ansi.COLOR_TAG_RGB;
    ...
    pub fn drawBox(
        ...
        if (shouldDrawTitle) {
            if (title) |titleText| {
                try self.drawTextWithTags(titleText, @intCast(titleX), @intCast(startY), borderColor, backgroundColor, 0, border_tag, background_tag);
            }
        }
    }
