...
pub fn TextBufferViewType(comptime BufferType: type) type {
    comptime {
        if (!@hasDecl(BufferType, "lineCount")) @compileError("BufferType missing lineCount");
        if (!@hasDecl(BufferType, "lineWidthAt")) @compileError("BufferType missing lineWidthAt");
        if (!@hasDecl(BufferType, "maxLineWidth")) @compileError("BufferType missing maxLineWidth");
        if (!@hasDecl(BufferType, "walkLinesAndSegments")) @compileError("BufferType missing walkLinesAndSegments");
        if (!@hasDecl(BufferType, "getTextRange")) @compileError("BufferType missing getTextRange");
        if (!@hasDecl(BufferType, "getPlainTextIntoBuffer")) @compileError("BufferType missing getPlainTextIntoBuffer");
        if (!@hasDecl(BufferType, "getLineSpans")) @compileError("BufferType missing getLineSpans");
        if (!@hasDecl(BufferType, "getSyntaxStyle")) @compileError("BufferType missing getSyntaxStyle");
        if (!@hasDecl(BufferType, "defaults")) @compileError("BufferType missing defaults");
        if (!@hasDecl(BufferType, "memRegistry")) @compileError("BufferType missing memRegistry");
        if (!@hasDecl(BufferType, "allocator")) @compileError("BufferType missing allocator");
        if (!@hasDecl(BufferType, "widthMethod")) @compileError("BufferType missing widthMethod");
        if (!@hasDecl(BufferType, "tabWidth")) @compileError("BufferType missing tabWidth");
        if (!@hasDecl(BufferType, "registerView")) @compileError("BufferType missing registerView");
        if (!@hasDecl(BufferType, "unregisterView")) @compileError("BufferType missing unregisterView");
        if (!@hasDecl(BufferType, "isViewDirty")) @compileError("BufferType missing isViewDirty");
        if (!@hasDecl(BufferType, "clearViewDirty")) @compileError("BufferType missing clearViewDirty");
        if (!@hasDecl(BufferType, "getContentEpoch")) @compileError("BufferType missing getContentEpoch");
        if (!@hasDecl(BufferType, "registerMemBuffer")) @compileError("BufferType missing registerMemBuffer");
        if (!@hasDecl(BufferType, "createChunk")) @compileError("BufferType missing createChunk");
    }
    ...
    return struct {
        ...
        fn calculateChunkFitWord(self: *const Self, chunk: *const TextChunk, char_offset_in_chunk: u32, max_width: u32) tb.ChunkFitResult {
            ...
            const wrap_offsets = chunk.getWrapOffsets(self.text_buffer.memRegistry(), self.text_buffer.allocator(), self.text_buffer.widthMethod()) catch {
                ...
                const WrapContext = struct {
                    ...
                    fn segment_callback(ctx_ptr: *anyopaque, _: u32, chunk: *const TextChunk, chunk_idx_in_line: u32) void {
                        ...
                        if (wctx.wrap_mode == .word) {
                            ...
                            const wrap_offsets = chunk.getWrapOffsets(wctx.text_buffer.memRegistry(), wctx.text_buffer.allocator(), wctx.text_buffer.widthMethod()) catch &[_]utf8.WrapBreak{};
...
pub const UnifiedTextBufferView = TextBufferViewType(UnifiedTextBuffer);
pub const StaticTextBufferView = TextBufferViewType(stb.StaticTextBuffer);
