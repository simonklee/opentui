...
/// StaticBackend serves read-only text with fast line queries and predictable
/// rebuild costs.
/// Use this backend when callers replace whole text values and render them many
/// times. Do not use it for incremental editing.
///
/// Data model:
/// - `segments` stores a flat stream of `linestart`, `text`, and `brk` markers.
/// - `line_starts[i]` stores the virtual offset where line `i` starts.
/// - `line_widths[i]` stores the display width of line `i` (newline excluded).
/// - `line_seg_start[i]..line_seg_end[i]` maps line `i` to its segment range.
///
/// Offset model:
/// - The backend tracks offsets in display-column space, not byte space.
/// - A newline contributes one virtual offset unit.
/// - `line_starts[i + 1] = line_starts[i] + line_widths[i] + 1`.
///   This keeps `line_starts` sorted, and enables binary search in
///   `getTextRange`.
///
/// Ownership and lifetime:
/// - `TextChunk` stores `(mem_id, byte_start, byte_end)` and never owns bytes.
/// - The caller owns `MemRegistry` buffers.
/// - The caller must keep each referenced `mem_id` active and stable while the
///   backend still references it.
/// - Chunk pointers exposed through callbacks point into `segments`.
///   Treat them as ephemeral, because rebuild paths can invalidate them.
///
/// Why `setText` uses prepare/commit:
/// - `prepareSetText` computes all required slots, and reserves capacity first.
/// - If reservation fails, the function returns an error, and old content stays
///   valid.
/// - `beginSetTextCommit` starts an infallible phase that only uses
///   `appendAssumeCapacity`.
///   This avoids partial writes under out-of-memory pressure.
///
/// Memory strategy:
/// - Global lists keep capacity across updates, which avoids repeated
///   allocations in steady state.
/// - The arena stores ephemeral chunk caches (graphemes and wrap offsets).
/// - `setText` and `reset` clear arena allocations in O(1) with `arena.reset`.
///
/// Invariants after every successful content update:
/// - The backend always reports at least one line.
/// - All line index arrays have the same length.
/// - If any segment exists, segment 0 is `linestart`.
/// - `max_line_width <= total_width`.
///
/// Example (`"A\r\nBC\n"`):
/// - `segments`: `[linestart][text("A")][brk][linestart][text("BC")][brk][linestart]`
/// - `line_widths`: `[1, 2, 0]`
/// - `line_starts`: `[0, 2, 5]`
///
/// Tradeoff:
/// - `appendFromMemId` returns `Unsupported` by design.
/// - Full replacement keeps the code simple, cache-friendly, and fast for
///   static content.
///
/// Why this fits snapshot-style streaming:
/// - Many render pipelines receive updates as full accumulated snapshots, not
///   incremental edit operations.
/// - This backend is tuned for that pattern. Each update rebuilds metadata from
///   scratch, and reuses retained capacity plus line-break scratch buffers.
/// - After warmup, many updates allocate no new heap memory for backend
///   metadata.
/// - If allocation still fails, prepare/commit keeps the previous snapshot
///   intact, so readers never see partial state.
/// - This backend only reduces buffer rebuild costs. View-layer wrapping and
///   virtual-line recomputation can still dominate for very large texts.
pub const StaticBackend = struct {
    ...
    /// clear removes all content, but keeps allocated capacities.
    ///
    /// This is cheap, and it keeps the backend ready for the next `setText`.
    /// It does not reset arena allocations.
    ...
    /// reset removes all content and resets arena-owned caches.
    ///
    /// Use this when you want a full memory reset without deinitializing the
    /// backend object.
    ...
    /// prepareSetText reserves every slot needed for a full text replacement.
    ///
    /// If this function returns an error, existing backend state stays intact.
    /// This gives `setText` a transactional prepare step.
    ...
    /// beginSetTextCommit starts the infallible commit phase after preparation.
    ///
    /// It clears current indexes, resets arena caches, and preserves list
    /// capacities so append operations cannot fail.
    ...
    /// walkLinesAndSegments visits lines in order, and emits text chunks for
    /// each line.
    ///
    /// The function skips marker segments, calls `segment_callback` for each
    /// text segment, then calls `line_end_callback` once per line.
    ///
    /// `chunk` pointers are valid for the callback call only.
    ...
    /// setTextFromMemId replaces the full backend content from one `MemRegistry`
    /// entry.
    ///
    /// The backend stores byte ranges, not copied bytes, so `mem_id` must stay
    /// active and stable while this content remains in use.
    ...
    /// setTextInternal replaces the full backend content from one mem buffer.
    ///
    /// The function builds segment and line indexes in one linear pass over
    /// precomputed line breaks.
    ///
    /// Important behavior:
    /// - CR, LF, and CRLF each become one logical break marker.
    /// - A trailing newline produces a trailing empty line.
    /// - The backend copies no text bytes. Chunks reference `MemRegistry`.
    ...
    /// extractChunkBetweenOffsets copies one chunk slice for a column range.
    ///
    /// The function maps column offsets to byte offsets with UTF-8 width rules,
    /// then copies at most what fits in `out_buffer`.
    ...
    /// rebuildLineIndex regenerates all derived line metadata from `segments`.
    ///
    /// Call this after chunk widths change (for example after `setTabWidth`).
    /// The function rewrites line starts, line widths, segment ranges, and all
    /// aggregate width and byte counters.
    ...
    /// getTextRange copies a `[start_offset, end_offset)` window into
    /// `out_buffer`.
    ///
    /// Offsets are virtual display offsets, where each newline counts as one
    /// offset unit.
    ///
    /// The function clamps `end_offset` to the current text length, returns the
    /// copied byte count, and truncates if `out_buffer` is too small.
    pub fn getTextRange(
        self: *const Self,
        mem_registry: *const MemRegistry,
        start_offset: u32,
