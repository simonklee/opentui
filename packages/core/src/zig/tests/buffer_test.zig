...
test "OptimizedBuffer - many unique graphemes with small pool" {
    ...
    var tb = try UnifiedTextBuffer.init(std.testing.allocator, &local_pool, .wcwidth, .rope);
    defer tb.deinit();
...
test "OptimizedBuffer - multiple TextBuffers rendering simultaneously should not leak" {
    ...
    var tb1 = try UnifiedTextBuffer.init(std.testing.allocator, &local_pool, .wcwidth, .rope);
    defer tb1.deinit();
    var view1 = try TextBufferView.init(std.testing.allocator, tb1);
    defer view1.deinit();

    var tb2 = try UnifiedTextBuffer.init(std.testing.allocator, &local_pool, .wcwidth, .rope);
    defer tb2.deinit();
    var view2 = try TextBufferView.init(std.testing.allocator, tb2);
    defer view2.deinit();

    var tb3 = try UnifiedTextBuffer.init(std.testing.allocator, &local_pool, .wcwidth, .rope);
    defer tb3.deinit();
