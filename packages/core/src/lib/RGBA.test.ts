...
describe("RGBA class", () => {
  ...
  describe("intent helpers", () => {
    ...
    test("normalizeColorValue preserves rgb, indexed, and default tags", () => {
      expect(normalizeColorValue(null)).toBeNull()
      expect(normalizeColorValue(RGBA.fromHex("#123456"))?.tag).toBe(COLOR_TAG_RGB)
      expect(normalizeColorValue(RGBA.fromIndex(12))?.tag).toBe(12)
      expect(normalizeColorValue(RGBA.defaultBackground())?.tag).toBe(COLOR_TAG_DEFAULT)
    })

    test("decodeColorTag decodes rgb, indexed, and default tags", () => {
